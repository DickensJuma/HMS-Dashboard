    ...
    def do_GET(self):
        ...
        if path == '/api/patients':
            if 'patient_id' in query_params:
                ...
                self.send_header('Content-type', 'application/json')
                self.end_headers()
                self.wfile.write(json.dumps(patients, cls=CustomJSONEncoder).encode())
            else:
                # List all patients
            ...
            if 'doctor_id' in query_params:
                ...
                self.send_header('Content-type', 'application/json')
                self.end_headers()
                self.wfile.write(json.dumps(doctors, cls=CustomJSONEncoder).encode())
            else:
                # List all doctors
    ...
    def do_PUT(self):
        ...
        if path == '/api/patients' and 'patient_id' in query_params:
            ...
            if result:
                ...
                self.send_header('Content-type', 'application/json')
                self.end_headers()
                response_data = {"code": 200,'message': 'Appointment information updated successfully', 'appointment_id': appointment_id}
                self.wfile.write(json.dumps(response_data).encode())
            else:
                self.send_response(404)
                self.end_headers()
